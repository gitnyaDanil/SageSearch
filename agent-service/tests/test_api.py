...
from api.server import app, agent
...
def test_create_and_poll_task_reports_missing_desktop_artifact_bridge():
    ...
    try:
        ...
        assert "not created locally" in final_data["error"] or "bridge" in final_data["error"].lower()
    finally:
        agent.mock_mode = orig_mock
